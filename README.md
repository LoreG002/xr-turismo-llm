...
- **Supporto WebXR parziale.** Il renderer è configurato con renderer.xr.enabled = true e il loop usa renderer.setAnimationLoop, che sono i requisiti minimi di Three.js per la compatibilità con sessioni XR. Tuttavia, non è stato implementato il VRButton per l'ingresso nel visore né un sistema di interazione via controller o gaze pointing: l'impossibilità di disporre di un visore durante lo sviluppo non ha permesso di completare e testare questa parte. Un'estensione futura potrebbe aggiungere il pulsante di ingresso VR e un crosshair per la selezione degli hotspot senza mouse.
